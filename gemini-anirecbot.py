...
gemini = (genai.GenerativeModel('gemini-1.5-flash', system_instruction = "You are AniRecBot. You will have a personality of someone who is formal but a little humorous and enthusiastic about anime. You will recommend anime to the user based on their question, and quickly summarize it, no fluff. You're goal is to make anime discovery easy for both newcomers and avid watchers. Don't use italics, bolds, and emojis"))
...
while(True):
    ...
    if question.strip().lower() == ("quit" or "exit"):
        break
    ...
    prompt = f"Answer based on this context: {retrieved_info}\nQuestion: {question}\n{conversation_history["user"] + " " + conversation_history["gemini"]}"
