...
#index = faiss.IndexFlatL2(knowledge_base_vector.shape[1])
#index.add(knowledge_base_vector)
...
while(True):
    ...
    #D, I = index.search(question_vector, k=2)
    #retrieved_info = [knowledge_base[i, for i in I[0]]

    #print(str(retrieved_info) + "\n")

    prompt = f"Answer based on this context: {retrieved_info}\nQuestion: {question}"

    results = pipe(prompt, max_new_tokens=200)
